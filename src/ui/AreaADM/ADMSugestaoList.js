...
export default function () {
	...
	async function getData() {
		setGridLoading(true)
		try {
			let { data } = await axios.get('http://localhost:3333/Sugestao')
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Sugestao/${deletable}`)
	...
	return (
		<>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Sugestões</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Sugestao/novo`)}>
					Nova Sugestão
				</Button>
			</Toolbar>
