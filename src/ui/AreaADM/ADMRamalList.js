...
export default function () {
	...
	async function getData() {
		setGridLoading(true)
		try {
			let { data } = await axios.get('http://localhost:3333/Ramal')
			...
			data.map((d) => {
				...
				d.dsColaborador = d.fkColaborador.dsColaborador
				delete d.fkColaborador
			})
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Ramal/${deletable}`)
	...
	const columns = [
		...
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/Ramal/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			<ConfirmDialog isOpen={dialogOpen} onClose={handleDialogClose}>
				Deseja realmente excluir este Ramal?
			</ConfirmDialog>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Ramais</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Ramal/novo`)}>
					Novo Ramal
				</Button>
			</Toolbar>
			<Paper elevation={4}>
