...
export default function () {
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Pabx/${deletable}`)
	...
	const columns = [
		...
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/Pabx/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Instruções PABX</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Pabx/novo`)}>
					Nova Instrução
				</Button>
			</Toolbar>
			<Paper elevation={4}>
