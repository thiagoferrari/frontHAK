...
export default function () {
	...
	async function getData() {
		setGridLoading(true)
		try {
			let { data } = await axios.get('http://localhost:3333/Login')
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Login/${deletable}`)
	...
	const columns = [
		...
		{
			field: 'dsEmailRec',
			headerName: 'Email Rec. (Esqueci Senha)',
			/* align: 'center',
			headerAlign: 'center', */
			flex: true
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/Login/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			<ConfirmDialog isOpen={dialogOpen} onClose={handleDialogClose}>
				Deseja realmente excluir este Login?
			</ConfirmDialog>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Logins do Portal Administrativo</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Login/novo`)}>
					Novo Login
				</Button>
			</Toolbar>
			<Paper elevation={4}>
