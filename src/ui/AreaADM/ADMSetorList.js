...
export default function () {
	...
	async function getData() {
		...
		try {
			...
			// Tratar Dados:
			data.map((d) => {
				d.nmFantasia = d.fkEmpresa.nmFantasia
				delete d.fkEmpresa
			})
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Setor/${deletable}`)
	...
	const columns = [
		...
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/Setor/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Setores</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Setor/novo`)}>
					Novo Setor
				</Button>
			</Toolbar>
			<Paper elevation={4}>
