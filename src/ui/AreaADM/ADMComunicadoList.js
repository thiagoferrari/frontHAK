...
export default function () {
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Comunicado/${deletable}`)
	...
	const columns = [
		...
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/Comunicado/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Comunicados</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Comunicado/novo`)}>
					Novo Comunicado
				</Button>
			</Toolbar>
			<Paper elevation={4}>
