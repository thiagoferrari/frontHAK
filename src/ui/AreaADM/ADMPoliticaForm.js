...
export default function ADMPoliticaForm() {
	const classes = useStyles()

	const [title, setTitle] = useState('Cadastrar Nova Política')
	...
	function handleDialogClose(result) {
		...
		if (result) history.push('/Politica')
	}
	...
	async function getData(id) {
		try {
			let { data } = await axios.get(`http://localhost:3333/Politica/${id}`)
	...
	async function saveData() {
		try {
			...
			if (params.id) await axios.put('http://localhost:3333/Politica', form)
			// Registro não existe, cria um novo (verbo HTTP POST)
			else await axios.post('http://localhost:3333/Politica', form)

			setSbOpen(true)
			setSbSeverity('success')
			setSbMessage('Dados enviados com sucesso.')
			setTimeout(() => { history.push('/Politica') }, 2000)
		}
		catch (error) {
			console.error(error)
			setSbOpen(true)
		...
		<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
			...
			<form className={classes.form} onSubmit={handleSubmit}>
				...
				<TextField
					...
					label="Título (Assunto da Política)"
				...
				<TextField
					id="dsResumo" InputLabelProps={{ shrink: true }}
					label="Texto de Descrição da Política" variant="outlined"
					color="secondary" value={form.dsResumo}
