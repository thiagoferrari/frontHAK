...
export default function () {
	...
	async function getData() {
		setGridLoading(true)
		try {
			let { data } = await axios.get('http://localhost:3333/Politica')
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3333/Politica/${deletable}`)
	...
	const columns = [
		...
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/Politica/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			<ConfirmDialog isOpen={dialogOpen} onClose={handleDialogClose}>
				Deseja realmente excluir este Política?
			</ConfirmDialog>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Políticas</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/Politica/novo`)}>
					Nova Política
				</Button>
			</Toolbar>
			<Paper elevation={4}>
